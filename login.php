  		...
  		<div id="login-right" class="bg-light">
			...
			&nbsp;<h4 style="text-align:center;"><b>LOGIN</b><h4>
  			<div class="card col-md-8">
				  ...
				  <button class="btn btn-primary float-right btn-sm" id="" onclick="openForm()"> Forgot Password?</button>
  			</div>
  			</div>
  		</div>
  ...
  <form action="<?php echo $_SERVER['PHP_SELF']; ?>" method="POST" class="form-container">
    <h1>Password Reset</h1>
		...
		<div class="form-group">
			...
			<input type="text" name="phone" id="name" class="form-control" placeholder="e.g 255*********" value="<?php //echo isset($meta['name']) ? $meta['name']: '' ?>" required>
		</div>
		
		<div class="form-group">
			<label for="username">New Password will be sent via sms to the number you registered in the system.</label>
    ...
    <button type="submit" name='smsg' class="btn">Request</button>
...
if ($message->getStatus() == 0) { 
    ...
    echo "<script> alert('Password request failed');</script>";
    ...
    echo "<script> alert('This number is not registered in the system contact administrator');</script>";
